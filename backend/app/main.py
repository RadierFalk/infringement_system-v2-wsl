...
async def validation_exception_handler(
    ...
    field = " -> ".join(
        str(loc) for loc in first_error["loc"] if loc != "body"
    )
    message = (
        f"{field}: {first_error['msg']}"
        if field
...
# Se colocássemos /api aqui, o Swagger/OpenAPI apontaria para o token URL errado.
app.include_router(auth_router)

# Todos os routers de domínio ficam sob /api.
